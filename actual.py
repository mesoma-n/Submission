...
# importing sys import sys 
# This is the main function def main():     
...
# we add the shift value to the pi value and do a % 26 so that if the value goes more than 26         # it can circle back between the range 0 to 26 
        ci = (pi + k) % 26        
...
# here chr() gives the ascii character corresponding to its number 
    ...
    return caesar_txt   
# check function whether the CLI argument for the caesar key K is given or not def check(arg):     # if only there are no two arguments or the second argument ie K is not an integer then exit 
